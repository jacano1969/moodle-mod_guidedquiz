<?php 

$string['assignvars'] = 'Assign vars';
$string['correctanswer'] = 'Correct answer';
$string['errornoargs'] = 'There are not guidedquiz type arguments in this question';
$string['errornoquizvars'] = 'The requested quiz does not have vars';
$string['finishattemptwithoutanswerlastquestion'] = 'Leave the question without answer and finish the attempt';
$string['mainquestionhidden'] = 'To view the guided quiz main question you must begin an attempt';
$string['modulename'] = 'Sections quiz';
$string['modulenameplural'] = 'Sections quizzes';
$string['nextquestion'] = 'Next question';
$string['nextquestionaccess'] = 'Trying to access a more advanced question';
$string['nextquestionwithoutanswer'] = 'Next question without answer';
$string['previousquestionaccess'] = 'Trying to access a previous question';
$string['questionattempts'] = 'Attempts';
$string['questionnumberof'] = 'Question {$a->this} of {$a->of}';
$string['questionpenalty'] = 'Penalty';
$string['remainingattempts'] = 'Remaining attempts';
$string['sendandfinish'] = 'Send and finish';
$string['vartypevar'] = 'variable';
$string['vartypeconcatvar'] = 'concat variable';
$string['viewpreviousquestions'] = 'View previous answered questions';
$string['showcorrectresponses'] = 'Show correct response/s';